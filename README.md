...
這是一套校內使用的「大學個人申請」報名系統，提供兩種登入機制（推薦採用 Workspace帳號），學生可以選填志願、封存並在整理後用於官網報名。資料儲存在 Google 試算表，維護簡單、流程清楚。

## 📄 建立副本
- 請將 [[大學個人申請校內報名系統v2](https://docs.google.com/spreadsheets/d/18LXX7tj5ifSaLuHkAWI8OroBFo_Iz7ldviTAwQI_5FA/copy)]的試算表複製到您自己的 Google 雲端硬碟，複製後檔案名稱可以任意修改 。
> [!IMPORTANT]
...
> [!IMPORTANT]
> 請下載甄選會提供的「限填一組或特殊報名限制校系一覽表」，使用網路上免費的PDF處理網站轉成Excel檔案，然後匯入到試算表中，再利用最上方`進階功能`的選單`限填一組`功能，程式就會自動幫您編組。
